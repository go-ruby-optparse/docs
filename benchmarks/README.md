...
# `go-ruby-optparse` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-optparse`
library** against the reference Ruby runtimes (MRI, MRI + YJIT, JRuby,
TruffleRuby). It measures each `OptionParser` **operation** through the Go API,
isolated from the rbgo interpreter, so the numbers answer: *is the pure-Go
`OptionParser` as fast as the reference runtime's own pure-Ruby `optparse` —
...
- `ruby/optparse.rb` — the equivalent workload; `ruby/_harness.rb` is the shared
  timer.
...
representative argv and register the **identical** eight options, and each op's
integer checksum is verified byte-identical across all runtimes
(`CHECK=1 go run .` / `CHECK=1 ruby ruby/optparse.rb`) before timing. Published,
dated results are in [`../docs/performance.md`](../docs/performance.md).
